        ...
        if restaurants[x]['type'] == 'unica': #Parser for Unica
            ...
            for day in soup.findAll('div', class_='accord'):
                ...
                try:
                    ...
                    for price in day.findAll('td', class_='price quiet'):
                        prices.append(price.string.encode('utf8').replace(";","").replace("\n", "").replace("	",""))
                    for menu in day.findAll('td', class_='lunch'):
                        foods.append(menu.string.encode('utf8').replace(";",""))
                        menuNr += 1
                    conn.execute("UPDATE {r} SET MENU = {m},PRICES = {p} WHERE ID = {id};".format(r=restaurants[x]['title'].encode('utf-8'), m="'"+"|".join(foods)+"'", wd="'"+dayName[dayNr]+"'",p="'"+"|".join(prices)+"'",w=week,id=int(str(dayNr)+str(week)+str(year))))
                    dayNr += 1
            ...
            for day in soup.findAll('div', class_='lunch-item-food'):
                ...
                for price in day.findAll('td', class_='price-student'):
                    prices.append((price.get_text()[:4 if len(price.get_text()) > 2 else 0:1] + 'e').encode('utf8').replace(";",""))
                dayNr = 0
                menuNr = 0
